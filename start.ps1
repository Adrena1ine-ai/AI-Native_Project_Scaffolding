...
Write-Host ""
Write-Color "========================================================" Cyan
Write-Color "    AI-Native Project Scaffolding                       " Cyan
...
try {
    ...
    Write-Color "    [OK] $pythonVersion" Green
} catch {
    Write-Color "    [ERROR] Python not found! Please install Python 3.10+" Red
...
if ($versionMatch.Success) {
    ...
    if ($major -lt 3 -or ($major -eq 3 -and $minor -lt 10)) {
        Write-Color "    [ERROR] Python 3.10+ required, found $major.$minor" Red
        exit 1
    }
}
...
try {
    ...
    Write-Color "    [WARN] Some dependencies may need manual installation" Yellow
}

# Check if port is available
$portInUse = Get-NetTCPConnection -LocalPort $Port -ErrorAction SilentlyContinue
if ($portInUse) {
    Write-Color "    [WARN] Port $Port is in use, trying $($Port + 1)..." Yellow
    $Port = $Port + 1
}
...
try {
    ...
    Write-Color "[ERROR] Failed to start. Try: python -m web.app" Red
    exit 1
}
